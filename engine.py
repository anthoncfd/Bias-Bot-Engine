...
def sync_spot_gold_and_get_history(supabase: Client) -> pd.DataFrame:
    ...
    if not GOLDAPI_KEY:
        ...
        try:
            ...
            # Corrected keys parsed from goldapi.io payload structure
            supabase.table("precious_metals_daily").upsert({
                "date": today_str,
                "metal": "XAU",
                "price": current_spot_price,
                "open_price": float(data.get("open_price", current_spot_price)),
                "high_price": float(data.get("high_price", current_spot_price)),
                "low_price": float(data.get("low_price", current_spot_price)),
                "prev_close": float(data.get("prev_close", current_spot_price)), # Fixed API key name conversion
    ...
    # Retrieve historical data from precious_metals_daily for XAU
    print("📥 Retrieving historical data cache from Supabase...")
    try:
        response = supabase.table("precious_metals_daily")\
            .select("date, price")\
            .eq("metal", "XAU")\
        ...
        df = df.rename(columns={'price': 'Close'}).set_index('date')
        return df
    except Exception as e:
...
def run_macro_engine():
    ...
    if not xau_df.empty and len(xau_df) >= MIN_REQUIRED_ROWS:
        ...
        supabase.table("macro_bias_summary").upsert(payload).execute()
